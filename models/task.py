...
class Task:
    ...
    def get_by_project(project_id, status=None):
        ...
        try:
            cursor = db.cursor(dictionary=True)
            query = """
            SELECT t.*, u_assign.username as assigned_to_name 
            FROM tasks t LEFT JOIN users u_assign ON t.assigned_to = u_assign.id WHERE t.project_id=%s
            """ 
    ...
    @staticmethod
    def mark_completed(task_id): #which automatically:
        # Sets status to 'completed'
        # Adds current timestamp to completed_at
