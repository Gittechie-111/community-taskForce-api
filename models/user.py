...
#1) CREATING/ADDING NEW USERS
    ...
    def create(username, email, password, full_name=None, phone=None, is_admin=False):
        ...
        try:
            ...
            #Hash the password
            password_hash = bcrypt.hashpsw(password.encode('utf-8'), bcrypt.gensalt())
            #Insert values into database
            cursor.exceute("""INSERT INTO users  (username, email, password_hash, full_name, phone, is_admin)
                           VALUES(%s, %s, %s, %s, %s, %s, %s)""",  (username, email, password_hash, full_name, phone, is_admin))
            #Save and get ID
            db.commit()
